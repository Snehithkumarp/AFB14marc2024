...
async def send_for_index(bot, message):
    ...
    if message.from_user.id in ADMINS:
        if (LAZY_MODE==True):
            ...
            buttons = [
                [ InlineKeyboardButton("📝✧ 💲𝚝🅰🌱𝚝 🌱𝓔𝚗🅰ᗰ🕴𝚗🐋 ✧📝", callback_data="rename") ],
                [ InlineKeyboardButton('📇✧✧  𝕊𝚝𝕒𝕣𝚝 𝕚ŋ𝕕ᗴＸ𝕚𝚗𝕘  ✧✧📇',callback_data=f'index#accept#{chat_id}#{last_msg_id}#{message.from_user.id}')],
    ...
    await bot.send_message(LOG_CHANNEL,
                           ...
                           reply_markup=reply_markup)
    if (LAZY_MODE == True):
        if message.from_user.id in MnT_RENAMERS:
            ...
            await message.reply('🎉\n\n\n❤️ Thank You For the Contribution, Wait For My Moderators to verify the files.\n\n\n🎁')
            buttons = [
                        [InlineKeyboardButton("📝✧✧💲𝚝🅰🌱𝚝 🌱𝓔𝚗🅰ᗰ🕴𝚗🐋 ✧✧📝", callback_data="requireauth") ],
                        [InlineKeyboardButton('⨳  𝘾𝙡𝙤𝙨𝙚 ⨳', callback_data='cancel')]]
            reply_markup = InlineKeyboardMarkup(buttons)
            file = getattr(message, message.media.value)
            filename = file.file_name
            filesize = humanize.naturalsize(file.file_size) 
            k = await message.reply(
                                f"\n⨳ *•.¸♡ 𝑴𝒏𝑻 ＭⓄｄ𝓔 ♡¸.•* ⨳\n\n🤩 Do you know MnTBot can do a lot of things at a time...\nWould you like to try some of it's amazing features... \n\n🪬Chat ID/ Username: <code>{chat_id}</code>\nℹ️Last Message ID: <code>{last_msg_id}</code> \n\n🎞**File Name** :- `{filename}`\n\n⚙️**File Size** :- `{filesize}`",
                                reply_to_message_id=message.id,
                                reply_markup=reply_markup)
            await asyncio.sleep(600)
            await k.delete()
